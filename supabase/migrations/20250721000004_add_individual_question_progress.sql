-- Add individual question progress tracking to participants table
-- This allows each player to advance through questions independently
...
BEGIN
  ...
  JOIN questions q ON quiz.id = q.quiz_id
  WHERE p.id = participant_uuid
  AND q.order_index = p.current_question_index
  LIMIT 1;
END;
$$ LANGUAGE plpgsql;
